          ...
          return (
            <div className="m-3 p-3 bg-accent-2 rounded-xl" key={index}>
              <div className="flex items-center">
                ...
                />
                <span className="pl-2 font-semibold text-base">{review.author_name}</span>
              </div>
              <div className="flex space-x-2 items-center">
                <span className="text-accent-4 text-sm">{review.rating} stars</span>
                <span className="text-accent-4 text-lg">
                  {'★'.repeat(Math.floor(review.rating))}
                </span>
                <span className="text-sm text-accent-4">{review.relative_time_description}</span>
              </div>
              <p className="text-sm">
                {review.text}
              </p>
